...
async function serialize(conn, message) {
    try {
        ...
        const isAdmin = isGroup
            ? groupMetadata?.participants?.some((participant) => participant.id === message.sender && participant.admin)
            : false;
        const isBotAdmin = isGroup
            ? groupMetadata?.participants?.some((participant) => participant.id === conn.user.jid && participant.admin)
            : false;
